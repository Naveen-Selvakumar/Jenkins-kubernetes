pipeline {
    ...
    environment {
        ...
        REGISTRY_CREDENTIALS = "git-terraform"  // Jenkins docker credentials ID 2!#
    }
        ...
        stage('Checkout Code') {
            steps {						
                withCredentials([usernamePassword(credentialsId: 'github-Naveen-Selvakumar', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {  //3!# git credentials ID
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/Naveen-Selvakumar/Jenkins-kubernetes.git", branch: 'main' //4!#git user-name repository ID
                }
            }
        }

x        stage('Build Docker Image') {
            steps {
